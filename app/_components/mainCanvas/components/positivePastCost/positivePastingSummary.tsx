...
import useFormStore from "@/store/positivePastingStore";
    ...
    return (
        <div className="text-sm w-[300px] flex flex-col justify-end p-2 text-white bg-gradient-to-bl from-gray-800 via-gray-700 to-gray-600 border border-gray-500 shadow-lg">
            <div className="px-2 mb-1">
                ...
                <p>{hindi ? "कुल प्लेट्स:" : "Total Plates:"} <strong>{totalPlates}</strong></p>
                <form onSubmit={formik.handleSubmit}>
                    <div className="mb-2 items-center flex justify-between">
                        ...
                        <div>
                            ...
                            />
                            <button type="submit" className="bg-black px-3 text-xs rounded-l-none text-white min-h-5 rounded">
                                {hindi ? "जोड़ें" : "Add"}
                            </button>
                        </div>
                    </div>
                </form>
                <p className="bg-yellow-400 text-black px-2 rounded-sm mt-2">
                    {hindi ? "प्रति प्लेट लागत:" : "Cost Per Plate:"} <strong>₹{costPerPlate}</strong>
                </p>
            </div>
        </div>
    );
